...
//
// NOTE: this is an intentional duplicate of .claude/scripts/fetch-orchestrator-prompt.mjs.
// Copilot/VS Code users may clone only the .github folder, so each tool references a
// copy in a folder it is guaranteed to have. Keep the two files in sync.
